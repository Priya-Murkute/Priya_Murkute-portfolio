...
Four generator scripts produce committed artefacts. None of them run during
...
npm run map         # src/data/worldDots.ts, the land dots of the About Me world map
```

`npm run map` is only needed to change the dot spacing or the map's extent (both
are constants at the top of `scripts/generate-world-dots.mjs`) — adding a place
doesn't touch it. It reads Natural Earth land from the `world-atlas` dev
...
│   │                          # the journey — an ordered list of stops the map draws and retraces (a stop can repeat)
│   ├── vision.ts              # Vision board: the goals (a `done` date puts one on the shelf), horizons, areas, affirmations
│   └── worldDots.ts           # GENERATED by `npm run map` — the world's land as a dot grid; don't edit
...
└── components/
    ...
    ├── FallingPetals.tsx      # petals drifting down through any positioned box (fall from lib/petalFall.ts): used by PetalScatter and the vision board
    ...
    │                          # hops the map closes in and pulls back out, until the visitor touches it. Under
    │                          # reduced motion the route is simply drawn.
    │                          # VisionBoard: three horizon columns of goals and a "Manifested" shelf, read-only (a goal's
    │                          # `done` date in data/vision.ts moves it to the shelf). Its progress meters fill and count up once seen, and petals
    │                          # drift behind it while it's on screen (FallingPetals). Quoted.tsx wraps every quotation in pink marks.
