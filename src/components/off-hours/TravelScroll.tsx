...
import { travelPhotos } from "@/data/gallery";
...
const HEIGHT_CLASS: Record<TravelPhoto["height"], string> = {
  tall: "h-[300px]",
  med: "h-[230px]",
  short: "h-[180px]",
};
    ...
    <div className="group relative overflow-hidden">
      ...
      >
        {loop.map((photo, index) => (
          <TravelPhotoCard key={`${photo.id}-${index}`} photo={photo} />
        ))}
      </div>
    </div>
  );
...
function TravelPhotoCard({ photo }: { photo: TravelPhoto }) {
  return (
    <div
      className={cn(
        "group/photo relative shrink-0 cursor-pointer overflow-hidden rounded-[3px]",
